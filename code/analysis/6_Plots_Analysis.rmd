...
q$big <- q$mname == "CC2MP6_DVARSdual_1185"

pdf(plt_fmt(paste0("NumScrubbed_vs_ICC_MAC_v2.pdf")), width=6.5, height=10)

p1 <- ggplot(q, aes(x=v2 + logjust, y=value, color=method)) + 
  scale_size_manual(values=c(2.0, 3.5)) +
  geom_vline(
    xintercept=logjust+c(13.886905, 27.877976), alpha=.5, size=1.5,
    color=color_m[["lightGray"]]) +
  geom_hline(yintercept=0, linetype="dashed") +
  ...
  scale_color_manual(values=color_m[levels(q$method)]) +
  scale_fill_manual(values=color_m[levels(q$method)]) +
  coord_cartesian(xlim=logjust + 1185* c(0, .1)) +
  scale_x_continuous(
    breaks=logjust + 1185 * c(0, .01, .02, .05, .1), 
    labels=paste0(" ", c(0, 1, 2, 5, 10), "%"),
    trans="log1p"
  ) + 
  scale_y_continuous(labels = function(q) format(q, scientific = TRUE)) +
  theme_bw() + 
  labs(color="Scrubbing method") +
  cowplot::theme_cowplot() +
  theme(legend.position = "bottom") +
...
s_quant <- c(.05, .25, .45, .65, .9)
snames <- meanFD[round(s_quant*nrow(meanFD)),]
snames$name <- paste0(
  snames$subject, "_v", 
  ((!snames$test) * 2) + snames$visit,
  "_", snames$acquisition
)
snames$quant <- s_quant

color_m2 <- c(modFD="#dd8080", ICA="#80affa")
```

```{r}
pdf(plt_fmt("ScrubMeas_Timecourse_just2.pdf"), width=85, height=4.5)

z <- setNames(as.list(snames$name), snames$name)
for (sname_ss in snames$name) {
  x <- list(
  ...
  z[[sname_ss]] <- tidyr::pivot_longer(y, seq(2,3))
...
```{r}
snames$sel_less <- c(
  162, # similar, but lev selects less
  455, # similar, but lev selects less # 577? lev, but not motion
  120, # similar, but lev selects less 
  542, # similar, but lev selects less 
  1059  # similar, but lev selects less 
)
snames$sel_lag <- c(
  911, 942, 282, 305, 528
)

for (what in c("less", "lag")) {
  pdf(plt_fmt(paste0("ScrubMeas_Timecourse_Selection_", what, ".pdf")), width=3, height=12)
  q <- vector("list", nrow(snames))
  for (ii in seq(nrow(snames))) {
    sname_ss <- snames$name[ii]
    ...
    df_flag_modFD <- data.frame(
      x = subset(z[[sname_ss]], value > 1 & name=="modFD")$idx,
      y=2, width=1, height=8
    )
    df_flag_ICA <- data.frame(
      x = subset(z[[sname_ss]], value > 1 & name=="ICA")$idx,
      y=2, width=1, height=8
    )
    df_flag_both <- rbind(df_flag_ICA, df_flag_modFD)[duplicated(rbind(df_flag_ICA, df_flag_modFD)),]
    
    z[[sname_ss]] <- subset(z[[sname_ss]], name %in% c("modFD", "ICA"))
    #z[[sname_ss]]$p <- z[[sname_ss]]$idx %in% (c(ok_ii, hilite_ii)+lims_ii)
    
    q[[ii]] <- ggplot(z[[sname_ss]], aes(x=idx, y=value, color=name)) + 
      geom_tile(
        aes(x=x,y=y, width=width,height=height), data=df_flag_ICA, 
        fill=color_m2["ICA"], color="#ffffff00", alpha=.45
      ) +
      geom_tile(
        aes(x=x,y=y, width=width,height=height), data=df_flag_modFD, 
        fill=color_m2["modFD"], color="#ffffff00", alpha=.45
      ) +
      geom_tile(
        aes(x=x,y=y, width=width,height=height), data=df_flag_both, 
        fill="#000000", color="#ffffff00", alpha=.12
      ) +
      geom_line(size=.7) + 
      ...
      theme_cowplot() +
      theme(legend.position="none") +
      coord_cartesian(xlim=c(-5,10)+snames[[paste0("sel_", what)]][ii], ylim=c(0, 6), expand=FALSE) +
      scale_x_continuous(breaks=seq(1185) * 1, labels=ifelse(seq(1185) %% 5 == 0, seq(1185), "")) +
      xlab("Timepoint") + ylab("Value/Threshold") + ggtitle(sname_ss)
  }
  print(cowplot::plot_grid(plotlist=q, ncol=1, align="hv"))
  dev.off()
}
```
...
p1 <- ggplot(subset(q, group %in% levels(group)[seq(10)]), aes(x=group, y=v)) + 
  ...
  scale_y_continuous(limits=c(-.03, .05), expand=c(0, 0)) +
...
p1 <- ggplot(subset(q, (!grepl("PCA", q$mname)) & group %in% levels(group)[seq(10)]), aes(x=group, y=v)) + 
  ...
  scale_y_continuous(limits=c(-.02, .025), expand=c(0, 0)) +
  cowplot::theme_cowplot() + theme(legend.position="none")
