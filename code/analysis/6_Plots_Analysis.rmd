...
toShow <- list(
  ...
  c("Baselines_None", "None__DCT0", "MPP"),
  c("Baselines_CC2MP", "CC2MP__06P_DCT4", "CC2MP6"),
  c("Baselines_CC5MP", "CC5MP__06P_DCT4", "CC5MP6"),
  c("Baselines_36P", "36P__whole_DCT4", "36P")
)
```

```{r}
bnames <- c("None", "CompCor", "CC2MP", "CSF-WM", "9P", "36P", "HCP__ICA-FIX")
q <- lapply(paste0("Baselines_", bnames), load_a_result, "meanFCz")
q <- do.call(cbind, do.call(cbind, lapply(q, function(x){x$meanFCz})))
colnames(q)[grepl("HCP__ICA-FIX", colnames(q))] <- "HCP__ICA-FIX"
q <- q[,!grepl("grayo", colnames(q))]
q <- q[,!grepl("DCT0", colnames(q))]
bnames <- colnames(q)
bnames2 <- gsub("_DCT4|__DCT4", "", bnames)
bnames2 <- gsub("CompCor__PC", "CC", bnames2)
bnames2 <- gsub("MP__06P", "MP06", bnames2)
bnames2 <- gsub("MP__24P", "MP24", bnames2)
bnames2 <- gsub("_withCblm|__whole", "", bnames2)
bnames2 <- gsub("HCP__", "", bnames2)
qB <- q
```

```{r}
bnames <- c(
  "9P_DVARSdual", "9P_FD___og_nfc_l4", "9P_Lev___ICA", "9P_Base",
  "CC2MP6_DVARSdual", "CC2MP6_FD___og_nfc_l4___", "CC2MP6_Lev___ICA", "CC2MP6_Base"
)
q <- lapply(bnames, load_a_result, "meanFCz")
q <- do.call(cbind, do.call(cbind, lapply(q, function(x){x$meanFCz})))
q <- q[,colnames(q) %in% c(
  "9P_DVARSdual_1185", "FD___og_nfc_l4___0.2", "Lev___ICA_kurt___3", "9P_Base_1185",
  "CC2MP6_DVARSdual_1185", "CC2MP6_Base_1185"
)]
bnames2 <- bnames <- c("9P_DVARS", "9P_modFD", "9P_pscrub", "9P_base", "CC2MP6_DVARS", "CC2MP6_modFD", "CC2MP6_pscrub", "CC2MP6_base")
qB <- q
colnames(qB) <- bnames2
```
...
#pdf(plt_fmt("FC_strength_grid.pdf"))
# nnmean <- vector("list", length=length(toShow))
# names(nnmean) <- vapply(toShow, `[[`, '', 3)
nnmean <- vector("list", length=length(bnames2))
names(nnmean) <- bnames2
for (ii in seq(length(nnmean))) {
  #q <- load_a_result(toShow[[ii]][1], "meanFC")
  #q <- q$meanFC[[1]][,toShow[[ii]][2]]
  q <- qB[,bnames[ii]]
  nnmean[[ii]] <- list(on=c(), off=c())

  # Get cor mat
  mat <- cor_mat(q, names=Labs$label, newOrder=order(Labs$idx2))[,rev(seq(419))]
  
  # Take network-network means in upper tri
  for (jj in seq(nrow(gg_pdv)+1)) {
    for (kk in seq(nrow(gg_pdv)+1)) {
      z <- mat[
        seq(pdv[jj], pdv[jj+1]-1), 
        seq(pdv[kk], pdv[kk+1]-1)
      ]
      z <- z[upper.tri(z)]
      if (jj==kk) {
        nnmean[[ii]]$on <- c(nnmean[[ii]]$on, z)
      } else {
        nnmean[[ii]]$off <- c(nnmean[[ii]]$off, z)
      }
    }
  }
}
...
z <- rbind(
  round(vapply(nnmean, function(x){t.test(x$on, x$off)$statistic}, 0), 5),
  round(vapply(nnmean, function(x){mean(x$on)-mean(x$off)}, 0), 5),
  round(vapply(nnmean, function(x){median(x$on)-median(x$off)}, 0), 5)
)
rownames(z) <- c("t.stat", "meanDiff", "medDiff")
z <- as.data.frame(t(z))
#dev.off()
```
...
```{r}
z$name <- factor(rownames(z), levels=rev(rownames(z)), labels=rev(gsub("_DCT4", "", rownames(z))))
ggplot(z, aes(y=name, x=t.stat)) + geom_bar(stat="identity") + theme_cowplot()
```
...
```{r}
nn <- nnmean
for (ii in seq(length(nn))) {
  nn[[ii]] <- data.frame(
    name = names(nn)[ii],
    v = do.call(c, nn[[ii]]) - median(nn[[ii]]$off),
    mdiff = median(nn[[ii]]$on) - median(nn[[ii]]$off),
    typeFC = c(rep("on", length(nn[[ii]]$on)), rep("off", length(nn[[ii]]$off)))
  )
}
nn <- do.call(rbind, nn)
nn$name <- factor(nn$name, levels=rev(bnames2), labels=gsub("_DCT4", "", rev(bnames2)))

z <- unique(nn[c("name", "mdiff")])
z$mdc <- as.character(round(z$mdiff, 3))
z$v <- .9

pdf(plt_fmt("FC_strength_ridge.pdf"), width=4.5, height=6)

ggplot(nn, aes(y=name, x=v, fill=typeFC)) + 
  geom_density_ridges(rel_min_height=.01, scale=1.55, quantile_lines=2, quantiles=2) + 
  theme_cowplot() + 
  geom_text(aes(y=name, x=v, label=mdc), data=z, inherit.aes = FALSE, hjust="left") +
  scale_fill_manual(values=c("#fc8d62", "#91d7eb")) +
  theme(legend.position="bottom") + xlab("Adjusted mean z-FC across subjects") + ylab("")
...
q$big <- FALSE
q$big[q$method=="FD" & q$cut==.2] <- TRUE
q$big[q$method=="modFD" & q$cut==.3] <- TRUE
...
```{r}
pdf(plt_fmt(paste0("NumScrubbed_vs_ICC_MAC.pdf")), width=5, height=15)
...
p1 <- ggplot(q, aes(x=v2 + logjust, y=value, color=method)) + 
  ...
  scale_x_continuous(
    limits=logjust + 1185* c(0, .5),
    breaks=logjust + 1185 * c(0, .01, .02, .05, .1, .2, .5), 
    labels=paste0(" ", c(0, 1, 2, 5, 10 , 20, 50), "%"),
    trans="pseudo_log"
  ) + theme_bw() + 
...
```{r}
pdf(plt_fmt("ScrubMeas_Timecourse.pdf"), width=80, height=4.5)

snames <- c("250427_v2_RL", "111312_v2_LR", "917255_v2_RL", "250427_v2_LR", "125525_v1_LR")
...
for (sname_ss in snames) {
  ...
  z[[sname_ss]]$name <- factor(z[[sname_ss]]$name, levels=c("FD", "modFD", "ICA", "DVARS"))
  q <- ggplot(z[[sname_ss]], aes(x=idx, y=value, color=name)) + geom_line() + 
    ...
    scale_x_continuous(breaks=seq(118) * 10) +
...
for (sname_ss in snames) {
  ...
  for (ii in seq(length(lims_ss))) {
    ...
    q <- ggplot(z[[sname_ss]], aes(x=idx - lims[[sname_ss]][ii], y=value, color=name)) + geom_line() + 
      geom_line(data=subset(z[[sname_ss]], name=="ICA")) +
      geom_hline(yintercept=0) +
      ...
      theme_cowplot() +
      theme(legend.position="none") +
      coord_cartesian(xlim=c(-10, 20), ylim=c(-.5, 4), expand=FALSE) +
      scale_x_continuous(breaks=seq(118) * 10) +
      xlab("Volumes after first motion peak?") + ylab("Value/Threshold") + ggtitle(sname_ss)
    print(q)
  }
}
dev.off()
...
```{r}
snames <- c("250427_v2_RL", "111312_v2_LR", "250427_v2_LR", "125525_v1_LR")
lims <- setNames(list(
  c(386, 584),
  c(727, 795, 892),
  c(102, 751),
...
pdf(plt_fmt("scrubbing_FC_strength_grid.pdf"))
for (ii in seq(length(mnames2))) {
  q <- load_a_result(paste0("CC2MP6_", mnames[[ii+1]]), "meanFC")
  q <- q$meanFC[[1]][,mnames2[[ii]]] - qB
  q <- ggcorrplot2(
    q, coloring$diverging7, divColor="white",
    paste("mean FC:", mnames3[[ii]]), 
    "mean FC", lim=lim_cFC
  )
  print(q)
} 
...
mnames2 <- c(
  ...
  "Lev___ICA_kurt___3", #"Lev___fusedPCA_kurt___5", "Lev___PCA_kurt___5",
  "DVARSdual"
)
mnames3 <- c("Base", "FD", "modFD", "ICA", "DVARSdual")

subs <- c("250427_v2_LR", "177746_v2_RL", "111312_v2_LR")

pdf(plt_fmt("FC_strength_grid_selSubjects.pdf"))

for (ii in seq(length(subs))) {
  q <- readRDS(file.path(
    dir_FC, "CC2MP6", paste0(subs[ii], ".rds")
  ))$FC[mnames2]
  q <- lapply(q, function(x){x$T_1185})
  
  for (jj in seq(length(mnames))) {
    z <- ggcorrplot2(
      q[[jj]], coloring$diverging7, divColor="black",
      paste("FC:", subs[ii], mnames3[jj]), 
      "FC", lim=lim_FC
    )
    print(z)
  }
...
pdf(plt_fmt("FC_strength_grid_selSubjects_diff.pdf"))
for (ii in seq(length(subs))) {
  q <- readRDS(file.path(
    dir_FC, "CC2MP6", paste0(subs[ii], ".rds")
  ))$FC[mnames2]
  ...
  for (jj in seq(2, length(mnames))) {
    z <- ggcorrplot2(
      q[[jj]]-q$Base, coloring$diverging7, divColor="black",
      paste("FC change:", subs[ii], mnames3[jj]), 
      "FC", lim=.05
    )
    print(z)
  }
...
```{r}
plt_idx <- "M"
agg <- readRDS(file.path(dir_FCval, "../3_FCval2", paste0("FC_CC2MP6.rds")))
...
### Validity exclusion analysis

```{r}
q <- readRDS(file.path(dir_FC, "CC2MP6/flag.rds"))
q <- lapply(q, function(x) {x[names(myLabs)]} )
q <- lapply(q, function(x) {do.call(cbind, x)})
q <- abind::abind(q, along=3)
q2 <- q; dim(q) <- c(dim(q)[1], dim(q)[2], 8, 42)
q <- q[,,seq(4),] + q[,,seq(5,8),]
dimnames(q) <- list(
  NULL,
  dimnames(q2)[[2]],
...
```{r}
nT1 <- 250
q2 <- q[vols_from_nT(nT1),,,]
q3 <- apply(q2, c(2,4), function(x){sum((2*nT1-colSums(x))*.72/60 < 5)})
nD1 <- rowSums(q3>0)
...
q3 <- apply(q2, c(2,4), function(x){sum((2*nT1-colSums(x))*.72/60 < 5)})
nD2 <- rowSums(q3>0)
subKeep <- which(colSums(q3[c(2,4,6,8,9,10,11,12),])==0)

z <- as.data.frame(cbind(nD1, nD2))
colnames(z) <- c("6 min.", "10 min.")
z <- z[c(1,3,5,7,2,4,6,8,9,10,11,12),]
rownames(z) <- c(
  "FD, 0.2", "FD, 0.3", "FD, 0.4", "FD, 0.5",
  "modFD, 0.2", "modFD, 0.3", "modFD, 0.4", "modFD, 0.5",
  "ICA, 3x", "ICA, 4x", "ICA, 5x",
  "DVARS"
)
```

```{r}
kableExtra::kable_styling(knitr::kable(z), 
...
```{r}
pdf(plt_fmt("FC_validity2.pdf"), width=7, height=8)

dfgg$group <- factor(
  ifelse(grepl("FD", dfgg$m2), "FD", dfgg$m2),
  levels=c("FD", "ICA", "DVARS")
)
dfgg$keep <- dfgg$sub %in% paste0("s", names(subKeep))

dfgg$mot2 <- factor(
  ifelse(dfgg$keep, as.character(dfgg$mot2), "Dropped"),
  levels=c("Low", "Mid", "High", "Dropped")
)
# 
...
#     ylab("Reduction in RMSE relative to denoising only") +
#     facet_grid(.~m2, scales="free_x", space="free")

dfgg <- subset(dfgg, m2!="FD")

ggplot(dfgg, aes(x=cut, y=crmse)) + 
    #ggnewscale::new_scale_color() +
    geom_line(data=subset(dfgg, mot2=="Low"), aes(group=sub, color=mot2), alpha=.5, size=.7) + 
    geom_line(data=subset(dfgg, mot2=="Mid"), aes(group=sub, color=mot2), alpha=.5, size=.7) + 
    geom_line(data=subset(dfgg, mot2=="High"), aes(group=sub, color=mot2), alpha=.5, size=.7) + 
    geom_line(data=subset(dfgg, mot2=="Dropped"), aes(group=sub, color=mot2), linetype="dashed", alpha=.5, size=.7) + 
    geom_boxplot(data=subset(dfgg, mot2!="Dropped"), fill="#ffffff", alpha=1, width=.25, outlier.shape = NA) + 
    geom_point(data=subset(dfgg, mot2=="Low"), aes(group=sub, color=mot2), size=1.5) + 
    geom_point(data=subset(dfgg, mot2=="Mid"), aes(group=sub, color=mot2), size=1.5) + 
    geom_point(data=subset(dfgg, mot2=="High"), aes(group=sub, color=mot2), size=1.5) + 
    geom_point(data=subset(dfgg, mot2=="Dropped"), aes(group=sub, color=mot2), size=1.5) + 
    geom_hline(yintercept=0, linetype="dashed") +
    #geom_point(aes(x=m, y=crmse, color=mot), size=2, alpha=.85) +
    #geom_violin(aes(x=m, y=crmse, fill=m), alpha=1) + 
    scale_color_manual(values=setNames(c("#42c1d2", "#268dd1", "#ac1f1a", "#444444"), levels(dfgg$mot2))) +
    scale_alpha_continuous(c(0,1)) +
    scale_y_reverse() +
    #scale_fill_manual(values=as.character(color_m[c("FD", "modFD", "FD" ,"modFD", "ICA")])) +
    #scale_color_manual(values=as.character(color_m[c("ICA", "FD", "modFD", "FD" ,"modFD")]))
...
```{r}
agg <- readRDS(file.path(dir_FCval, "../3_FCval2", paste0("FC_CC2MP6.rds")))
#agg <- sqrt(apply(agg^2, c(3,4), mean))
agg <- (apply(abs(agg), c(3,4), median))
```
...
pdf(plt_fmt("FC_validity_grid_scrubbing_MAD.pdf"))
for (ii in seq(length(mnames2))) {
  q <- (agg[1,] - agg[mnames2[ii],])
  #print(mnames3[ii])
  #print(quantile(q, 1-c(.99999, .9999, .999)))
  q <- ggcorrplot2(
    q, coloring$diverging4, divColor="white",
    paste("Reduction in MAD:", mnames3[ii]), 
    "Reduction in MAD", lim=.005, diagVal=0
  )
  # q <- ggcorrplot2(
  #   q, coloring$diverging4, divColor="white",
  #   paste("RMSE Reduction:", mnames3[ii]), 
  #   "RMSE Reduction", lim=.2, diagVal=0
  # )
  print(q)
} 
dev.off()
