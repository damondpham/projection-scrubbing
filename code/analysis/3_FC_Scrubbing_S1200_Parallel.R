...
for (baseName in baseNames) {
  ...
  foreach::foreach(ii = seq(nrow(iters))) %dopar% {
    library(ciftiTools)
    ...
    if (skip_ii(FC_fname)) { return(NULL) }

    # Scrubbing measures
    lev_fname <- file.path(lev_dir, paste0("LEV_", suffix, ".rds"))
    missingScrubbing <- (!file.exists(lev_fname))
    if (!missingScrubbing) {
