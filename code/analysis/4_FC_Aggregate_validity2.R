...
for (baseName in baseNames) {
  ...
  all_FCval_fname <- file.path(dir_FCval, "../3_FCval2", paste0("FC_", baseName, ".rds"))
  ...
  for (nn in seq(length(subjects))) {
    ...
    sess_nn <- file.path(dir_FCval, "../3_FCval2", baseName, paste0(sess_nn, ".rds"))
  ...
  saveRDS(agg2, file.path(dir_FCval, "../3_FCval2", paste0("FC_", baseName, "_mean.rds")))
}
